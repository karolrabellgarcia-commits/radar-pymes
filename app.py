...
# --- CONFIGURACIÓN DE PÁGINA (FAVICON EJECUTIVO) ---
st.set_page_config(
    page_title="KROMA Ops | Strategic Foresight & Operations Intelligence",
    page_icon="🔷",
    layout="wide",
    initial_sidebar_state="collapsed",
)

# Control de navegación interna (Landing vs. Terminal)
if "pantalla_activa" not in st.session_state:
    st.session_state["pantalla_activa"] = "landing"

# --- ESTILO VISUAL TRENDONE / LUXURY DARK TECH ---
st.markdown(
    """
    <style>
...
# --- CABECERA SUPERIOR PERMANENTE (KROMA OPS) ---
c_logo, c_nav = st.columns([2, 1])
with c_logo:
    st.markdown(
        """
        <div style="display: flex; align-items: center; gap: 12px; padding: 6px 0 16px 0;">
            ...
            <span style="font-size: 12px; color: #64748b; margin-left: 8px;">| Operational Foresight Engine</span>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
with c_nav:
    st.markdown(
        """
        <div style="display: flex; justify-content: flex-end; align-items: center; gap: 8px; padding-top: 8px;">
            <span style="font-size: 11px; font-weight: 600; color: #047857; background: #ECFDF5; padding: 3px 9px; border-radius: 20px; border: 1px solid #A7F3D0;">
                ● Residuo 0,00 €
            </span>
            <span style="font-size: 11px; font-weight: 500; color: #475569; background: #F1F5F9; padding: 3px 9px; border-radius: 20px; border: 1px solid #E2E8F0;">
...
def generar_pdf_editorial(markdown_texto, empresa_nombre, sector_nombre):
    ...
    <body>
        <div class="portada">
            <div class="portada-subtitulo-sup">KROMA OPS — Strategic Intelligence Dossier</div>
...
# =====================================================================
# PANTALLA 1: LANDING HERO (LOOK TRENDONE / ESTRATÉGICO)
# =====================================================================
if st.session_state["pantalla_activa"] == "landing":
    st.markdown(
        """
        <div class="kroma-landing-hero">
            <div class="kroma-badge-pill">Strategic Foresight & Capacity Check-up</div>
            <div class="kroma-title-gradient">Transforma Capacidad Instalada<br>en Margen Neto de Explotación</div>
            <div class="kroma-sub-hero">
                La plataforma cuantitativa de diagnóstico para pymes de instalaciones y servicios técnicos (SAT).
                Sustituye la intuición por modelos de datos auditados con residuo cero, 
                radar de tendencias sectoriales y cuenta proforma a 36 meses.
            </div>
        </div>
        """,
        unsafe_allow_html=True,
    ...
    with b1:
        st.markdown(
            ...
            <div class="bento-card">
                <div class="bento-tag">01. Diagnóstico de Tiempos</div>
                <div class="bento-title">Fugas de Capacidad</div>
                <div class="bento-desc">
                    Cuantifica el coste salarial improductivo absorbido en albaranes manuales, desplazamientos de mostrador y presupuestos sin cerrar.
                </div>
            </div>
            """,
            unsafe_allow_html=True,
    ...
    with b2:
        st.markdown(
            """
            <div class="bento-card">
                <div class="bento-tag">02. Modelo Financiero</div>
                <div class="bento-title">Residuo Cero (0,00 €)</div>
                <div class="bento-desc">
                    Conciliación matemática estricta entre palancas operativas y cuenta de pérdidas y ganancias proforma sin márgenes ficticios.
                </div>
            </div>
            """,
            unsafe_allow_html=True,
    ...
    with b3:
        st.markdown(
            """
            <div class="bento-card">
                <div class="bento-tag">03. Marco Regulatorio</div>
                <div class="bento-title">Blindaje 2026 / 2027</div>
                <div class="bento-desc">
                    Adaptación técnica oficial a la Ley Crea y Crece (RD 238/2026 Facturae/UBL) y al reglamento Veri*factu (RD 1007/2023).
                </div>
            </div>
            """,
            unsafe_allow_html=True,
    ...
    with c_btn2:
        if st.button("Iniciar Check-up Operativo y Plan de Capacidad (4 min) →", type="primary", use_container_width=True):
            st.session_state["pantalla_activa"] = "terminal"
            st.rerun()

# =====================================================================
# PANTALLA 2: TERMINAL DE DATOS, GENERACIÓN Y DOSSIER
# =====================================================================
else:
    c_back1, c_back2 = st.columns([4, 1])
    with c_back1:
        st.markdown("### Terminal de Evaluación Operativa y Modelado")
        st.caption("Completa los datos de estructura y costes para ejecutar el motor de inteligencia de KROMA Ops.")
    with c_back2:
        if st.button("← Volver a Portada", use_container_width=True):
            st.session_state["pantalla_activa"] = "landing"
            st.rerun()

    # --- BARRA LATERAL ---
    with st.sidebar:
        st.markdown("### Acceso KROMA Engine")
